                    ...
                    source: function source(query, callback) {
                        ...
                        if (searchConfig.type === 'lunr') {
                          var search = function search() {
                            ...
                            // Usa Fuse.js per cercare anche su sottostringhe
                            var fuse = new Fuse(Object.values(_this3._indexData), {
                              keys: ['title', 'content'],
                              threshold: 0.4,  // Modifica la soglia per risultati più o meno precisi
                            });
                        
                            // Cerca con Fuse.js (che supporta sottostringhe)
                            var fuseResults = fuse.search(query);
                            fuseResults.forEach(function(result) {
                              var uri = result.item.uri;
                              var title = result.item.title;
                              var context = result.item.content;
                        
                              // Aggiorna i risultati come hai fatto prima con Lunr
                              results[uri] = {
                                'uri': uri,
                                'title': title,
                                'context': context
                            ...
                            // Integrazione con Lunr per l'ordinamento
